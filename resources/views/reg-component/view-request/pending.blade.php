     ...
     <div class="sm:flex sm:justify-between">
         <div>
             ...
             <p class="max-w-2xl mt-1 text-sm text-gray-500">
                 Current status: @if ($this->request->status == 'finalized')
                     <span class="text-yellow-600 uppercase">Pending</span>
                 @endif
             </p>
         </div>
             ...
             @if ($this->request->requestor_current_status == 'graduated')
                 @if (auth()->user()->campus_id != '1')
                     <div>
                         <x-button primary
                             icon="fast-forward"
                             wire:click="forwardRequest"
                             spinner="forwardRequest">
                             Forward to ACCESS Campus
                         </x-button>
                     </div>
                     <div>
                         <x-button negative
                 ...
                 @else
                     <div>
                         <x-button wire:click="approvedRequest"
                             icon="thumb-up"
                             info>
                             Approve
                         </x-button>
                     ...
                     <dl class="sm:divide-y sm:divide-gray-200">
                         <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                             ...
                             <dd class="mt-1 text-sm text-gray-900 uppercase sm:mt-0 sm:col-span-2">
                                 {{ $this->request->requestor_current_status }}
                             </dd>
                         </div>
                         <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                             <dt class="text-sm font-medium text-gray-500">
